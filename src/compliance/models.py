...
class Purpose(str, Enum):
    ...
    Three are modelled, and deliberately so: **no purpose's scope contains
    another's**. Care coordination may see clinical data but not financial or
    contact data; billing settlement may see financial and contact data but not
    clinical data; patient registration may see contact and quasi-identifying
    data but neither clinical nor financial data. That pairwise non-nesting is
    the point. It makes "out of scope" mean *not necessary for this purpose*
    rather than *more sensitive in general*, which is what the DPDP
    purpose-limitation principle actually says, and it lets the rules
    discriminate in every direction instead of ranking purposes on a single
    axis of permissiveness. ``tests/compliance/test_purpose_matrix.py`` asserts
    the property, so a fourth purpose cannot quietly break it.
    """
