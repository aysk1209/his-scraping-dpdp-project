...
- **Not started:** the rule-based staff-guidance agent (`src/agent/` is a docstring); real Tier 2 browser code (`src/extraction/tier2/` is empty); the rough mock portal; the dataset adapter; full per-layer pydantic schemas; a real AutoScraper baseline.
