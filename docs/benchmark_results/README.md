...
| `rules-vs-just-ai.html` | `python tools/build_demo_page.py` after either benchmark (rendered from `tools/rules_page.html` with the shared demo-page look, 2026-09-26) | **The Review-II demo page**: five steps; a briefing switch and a shared legend that overlays or hides techniques on every chart; a radar of the five axes; a tab on where "just AI" fails (traps, retention, coverage, stability); per-rule overlay; field matrix per task; the numbers. |
