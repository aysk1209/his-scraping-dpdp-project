...
| `rules-vs-just-ai.html` | hand-built from the benchmark JSONs and the recordings (2026-09-17) | **The Review-II demo page**: tabs; a shared legend that overlays or hides techniques on every chart; a radar of the five axes; a tab on where "just AI" fails (traps, retention, coverage, stability); per-rule overlay; field matrix per task; the numbers. |
