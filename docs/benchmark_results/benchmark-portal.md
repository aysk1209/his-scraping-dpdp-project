...
_compliance-aware (ours) scores 1.000; unconstrained (baseline) scores 0.135 on the same 7 rules -- a 0.865 gap. It also pulls 1.00x the fields the purpose requires, against 7.15x for unconstrained (baseline) at full coverage. That surplus is exactly what the data-minimisation rule penalises, so on this workload compliance and extraction cost move together rather than trading off against each other. ai agent: gemini (told the Act) obtained only 77% of the fields the tasks require: it left out data the purpose lawfully needed, so its low cost is a shortfall, not efficiency._

Source: portal, 20 records/module, seed 42. 3 extraction tasks, identical DPDP rule set for every technique. Generated 2026-09-16 in 39181 ms (wall-clock, hardware-dependent).
...
|---|---|---|---|---|---|---|---|---|---|---|
| compliance-aware (ours) | 1.000 | 1.00 | 1.00 | 13 / 13 | 260 | 5 | 70 | 100 | 5387.1 | n/a |
| ai agent: gemini (told the Act) | 0.988 | 1.31 | 0.77 | 17 / 13 | 340 | 6 | 72 | 120 | 5478.6 | n/a |
| ai agent: gemini (unaided) | 0.976 | 1.31 | 0.85 | 17 / 13 | 340 | 5 | 70 | 100 | 4953.5 | n/a |
| unconstrained (baseline) | 0.135 | 7.15 | 1.00 | 93 / 13 | 1860 | 15 | 330 | 300 | 23358.0 | n/a |

**Per task**

| Task | compliance-aware | gemini-informed | gemini-unaided | unconstrained |
|---|---|---|---|---|
| `patient-summary` | 1.000 | 0.964 | 0.964 | 0.131 |
...
- **gemini-informed** — 120 records across 2 layer(s); records carrying each category: administrative (100), clinical (20), contact (20), direct_identifier (60), quasi_identifier (20); out-of-scope: none
- **gemini-unaided** — 100 records across 2 layer(s); records carrying each category: administrative (100), clinical (20), contact (20), direct_identifier (60), quasi_identifier (20); out-of-scope: none
