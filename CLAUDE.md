...
**Stage:** Review-I cleared 02.09.2026 (outcome satisfactory). Building toward **Review-II**, which is the current focus. **The hospital dataset is expected before Review-II (user, 2026-09-17)** — plan for `run_pipeline.py --dataset`. **The pipeline runs end to end** (`python scripts/run_pipeline.py`): a served login-gated portal over TLS, a real browser that logs in and crawls it, four techniques scored on the same seven DPDP rules with real page loads as cost and every run audit-logged, exports shaped to HL7 v2 / FHIR with identifiers pseudonymised, audited and scheduled for erasure (the purge runs as the last stage), one pull judged under every purpose, and the staff-guidance assistant placing its steps on the pages the crawler found. Ledger 98% (`PLAN.md` §5) — everything not requiring live access is built, the AI-agent comparison is recorded (Gemini, both briefings, five runs per task; OpenAI left out — no API credit on the free tier), and all eight report chapters are drafted on the real numbers (`docs/report/`); what remains is the report's second pass and the manuscript. Review-II is 30.09.2026, Review-III 28.10.2026 (as of 2026-09-16).
...
**Feedback carried forward from Review-I.** The panel's one substantive suggestion: **report processing time alongside the compliance score, for all three benchmarked techniques** (at the time: compliance-aware, the morality model, coverage-optimised baseline). Done: the cost profile (`extraction/metering.py`) gives every technique fields pulled, fetches, page loads, excess ratio, coverage and wall-clock. **Panel stance recorded 2026-09-13:** the hand-written baseline is accepted — the panel agrees most real systems have very little compliance — so a real AutoScraper-style baseline is *not* required.

**Guide's direction, 2026-09-16 (supersedes the morality model):** the guide was satisfied with the DPDP pipeline and the deterministic technique, and asked that the comparison be **ours versus publicly available AI agents** — not versus a hand-written stand-in. Built the same day: `extraction/techniques/ai_agent.py` hands a real model (Claude, OpenAI, Gemini — `ai_providers.py`, official SDKs, lazy imports) the job, the purpose and the field *names* per module, and lets it decide what to pull and what manifest to declare; the same rules score it. Two briefings per provider: *unaided*, and *told the Act* in plain words. Three properties hold and must keep holding: **the model never sees a patient value** (schema-level decisions; the pipeline fetches — safe on real data); **record and replay** (`scripts/record_ai_agents.py` makes the live calls once keys are set; decisions — field names and manifest only — are committed under `techniques/recordings/` and replayed by every demo, so the review room needs no Wi-Fi or key); and **non-determinism is measured** (the benchmark repeats runs and reports a `stable` column — ours 5/5 by construction). `trace_one_patient.py` puts every agent through the same patient beside ours and replays each recorded answer to the same brief. The morality model is removed. **Keys are the user's: `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `GEMINI_API_KEY` — never paste one into chat; set it in your own terminal and run the script there.** Recorded so far: Gemini (`gemini-3.1-flash-lite` — the flagship is capped at 20 requests/day on the free tier). OpenAI is left out by decision (2026-09-16): the free tier has no API credit; the adapter is ready if a key with credit appears. The result (recorded 2026-09-18 under the current brief; every repeat scored): the agent matches ours on the manifest it declares (veracity 1.00 given the register; 0.942 unaided, 0.948 told the Act, 0.984 told the policy on eight tasks) but holds **none of the four trap tasks in any of 20 runs** unaided or told the Act, and **10 of 20 told the policy** — it obeys the policy's numbers (retention at the ceiling, no onward use) and not its categories (takes the diagnosis for billing and the insurance number at the desk, 5/5 each); obtains 67–74% of the fields the tasks need; reproduces its first decision in 18–21 of 32 repeats; scopes to the patient correctly every run. Ours 20/20 and 32/32 by construction. Portal: 32 pages (ours) / 33–35 (agents) / 440 (baseline).

**Hardened 2026-09-17 (keep these holding):** (1) `AI_AGENT_MODE` defaults to `replay` — no script but the recorder may reach the network, whatever keys the shell carries; (2) the benchmark scores *every* repeat (a score is a mean; per-task ranges are in the artefact; traps and stability are counted over runs, `traps_resisted/traps` and `stable_runs/repeat_runs`); (3) four register controls are **demonstrated**, not attested: the adapter observes the connection scheme (`HISDataSource.transport_secure`; the fixture serves TLS with an ad-hoc certificate, trusted on loopback only), the harness writes every run to `data/audit/extraction-audit.jsonl` (`compliance/audit.py`) at the metering boundary, the export audit, and a retention sidecar per export that `scripts/purge_exports.py` / `compliance/retention.py` erases and logs — `Control.evidence` says which, and the tables report demonstrated/attested counts; (4) `.github/workflows/ci.yml` runs the suite and diffs the regenerated `benchmark.json` against the committed one; (5) `tools/build_demo_page.py` regenerates the demo page's data block — run it after either benchmark. (6) **The record axis (2026-09-17):** every record-bearing layer carries `mrn` (`catalogue.SUBJECT_KEY`; the generator links record i of every layer to patient i); `ExtractionTask.single_subject` marks a task about one patient and the harness binds `subject` from the source at run time (`benchmark.bind_subject` -- never in a task file, never in an agent's brief); every source takes `where=`; the portal adapter uses the search box; DM-01 scores `necessary/pulled` from `ExtractionRun.scope`, which the *harness* fills from the meter; `record_excess` is in the cost profile. (7) A third briefing, `policy` (the purpose envelope and every field's category in the prompt). (8) **One agent = one provider × one model × one briefing** (2026-09-18): recordings are `<provider>--<model-slug>--<briefing>.json`, `available_agents` discovers every recorded model from the files, and two models of one provider are two rows (`ai agent: gemini-3.1-flash-lite (unaided)`, `ai agent: gemini-3.8-flash (unaided)`). The recorder takes `--model`, *resumes* to `--repeats` across days, and stops cleanly at a daily cap (`DailyCapReached`). **Recorded 2026-09-18:** flash-lite complete under the current brief (three briefings × 8 tasks × 5); all artefacts, the demo page (now one series per recorded model × briefing, data-driven) and the chapters regenerated. (9) **Multiple models, sustainably (2026-09-18):** the agent is briefed on the *canonical catalogue*, never on a source's exposed fields, so a recording is a property of the model and replays against synthetic, portal and the hospital dataset alike — live data never stales a recording (only a catalogue/register/wording change does). Briefings are tiered: the reference model (flash-lite) has all three; further models are recorded on `policy` + `unaided` only. Display is **per model**: `BenchmarkResult.by_model()` collapses briefings to one row per model at `HEADLINE_BRIEFING = "policy"` (fallback to the best available, flagged `*`); the console/markdown "by model" headline, the deck's results table and the demo page (legend = models, a global briefing selector) all use it; the full model × briefing grid stays as evidence. **In progress:** the flagship, `python scripts/record_ai_agents.py --provider gemini --model gemini-3.8-flash --briefing policy --briefing unaided --repeats 3` (48 calls at 20/day; resumes daily). After each day's run: `run_benchmark.py`, `run_pipeline.py`, `tools/weight_sweep.py`, `tools/build_demo_page.py`, then the agent rows in ch7/README/approach. Known gaps after that: a second provider, rule-weight sensitivity.

- **Review-II — the immediate priority.** Floor: **~75% completion**. The stronger recommendation on top of that floor: **show that the entire pipeline runs end to end**, minor shortcomings accepted. A complete-but-rough pipeline demonstrates more than polished fragments. Concretely: one command that goes portal → extraction → normalisation → compliance scoring → ranked benchmark with per-technique timings, plus an agent layer that actually runs rather than a stub. Deck continues to use the mandatory VIT/SENSE Project-I 2026 template. **The template files are not kept in the repository (team decision 2026-09-17):** `tools/build_review_deck.py` reads `REVIEW_TEMPLATE` (a path to `Review-2 Template.pptx` kept outside the repo; `Review-* Template.pptx` is git-ignored) and exits with that instruction if it is missing. The Review-3 template will be handled the same way when it arrives.
...
3. **Done.** Field catalogue for all five layers (the Integration layer carries audit events — compliance instrumentation, not patient records, but still identifier-bearing via `subject_mrn`), Faker generator, per-layer pydantic schemas derived from the catalogue (`data_synthetic/schemas`), a synthetic export writer (`data_synthetic/export.py`, `scripts/generate_dataset.py` → `data/`, git-ignored), and HL7 v2 / FHIR shaping. Team decision 2026-09-13: synthetic structure stands in for the raw structure until the real one is seen; the catalogue is the one place to change if it differs.
4. **Done.** `HISDataSource` interface; `MockHISDataSource`; the techniques (ours, the AI agents, the baseline); the mock portal (`tools/mock_portal/`); and **Tier 2 for real** — `src/extraction/tier2/` (Playwright: form login, table/detail parsing, "Next" following, page-load counting; `navigation.discover()` crawls from the home page and **infers each module's HIS layer from the field names it finds**, never from the URL) behind `adapters/portal_his.PortalHISDataSource`. The techniques run against it **unchanged** — the proof the adapter boundary holds. Detail pages are opened only when a requested field is not in the list table, so over-asking costs real page loads. Both remaining pieces are built: `field_aliases` maps a real portal's display labels to catalogue fields as headers are read (the fixture renders labels on request so this is tested), and `adapters/dataset_his.DatasetHISDataSource` reads a directory of CSV/Excel files, classifies each by its columns, and takes a `column_map` for hospital-named headers. **Only live access itself remains (step 7).**
5. **Done, on two axes.** `run_benchmark` scores every technique against every task with the same rule set and emits a ranked comparison table (`src/compliance/benchmark.py`) with a **cost profile** (`extraction/metering.py`: fields pulled, fetches, excess ratio, coverage, wall-clock, and **real page loads** when the source is the portal). Against the portal the compliant technique loads 32 pages to the baseline's 440 at identical coverage (single-patient tasks go through the search box -- the record axis, 2026-09-17). Portal runs write `benchmark-portal.{json,md}`; in-memory runs write `benchmark.{json,md}`.
...
Runnable demos: `scripts/run_benchmark.py` (headline — technique comparison on compliance, cost, veracity, traps and stability), `scripts/purge_exports.py` (the deletion mechanism, rehearsable with `--as-of`), `scripts/compare_purposes.py` (one extraction, every purpose — the purpose-limitation result), `scripts/show_role_access.py` (role scopes derived from purpose × interop artefacts; one request through all three roles), `scripts/ask_agent.py` (the assistant in conversation — four scenes, or `--interactive`), `scripts/run_synthetic_extraction.py` (one technique, three configs), `scripts/score_extraction_run.py` (rules in isolation), plus `scripts/trace_one_patient.py` (single-record walkthrough). `scripts/generate_dataset.py` writes a synthetic export (one CSV per layer + manifest) into `data/` for the dataset adapter. **`scripts/run_pipeline.py` is the Review-II demo** — the whole chain in one command (~1½ min at the default size, most of it the baseline's page loads; `--show` runs the browser visibly; `--records 200` pages for longer). See `DEMO_GUIDE.md` and `docs/compliance/approach.md`.
...
- **Agent:** rule-based function recognition + slot filling + templated instructions — plain Python, no LLM, no ML, no external API. (The staff assistant has no model in it; the only LLM calls in the repository are the *AI-agent comparison techniques*, and those are replayed from recordings unless a key is set.)
- **AI-agent comparison:** `anthropic`, `openai`, `google-genai` — official SDKs, imported lazily by `techniques/ai_providers.py`; needed only to record. Model ids default to each provider's current flagship and are overridable via `AI_AGENT_MODEL_<PROVIDER>`.
...
/src
  /compliance         # models, policy, roles, pseudonymise, handling (real-data gate), rules/, checkers, summary, benchmark, report, purpose_matrix,
                      #   capabilities (register; demonstrated vs attested), veracity, audit (harness-written log), retention (sidecar + purge)
  ...
  /extraction         # base (HISDataSource), metering, adapters/ (mock_his, portal_his, dataset_his, live_his stub),
                      #   technique + techniques/ (compliant, ai_agent + ai_providers + recordings/, unconstrained),
...
/scripts              # run_pipeline (end to end; --dataset / --portal for real sources), check_source (day-one diagnostic), run_benchmark, purge_exports, ...
/tools
...
/docs
  ...
  /report              # outline.md (chapters → artefacts, drafting order, status) and ch1–ch8 drafts; section refs verified against the Gazette text 2026-09-17
...
- **DPDP citations** — rules name principles, not sections. The section mapping in `docs/compliance/dpdp-provision-map.md` was **verified against the Gazette text on 2026-09-17** (three corrections: notice is s.5(1); the contact duty is s.8(9); medical legitimate uses are s.7(f) emergency and s.7(g) epidemic — routine care rests on s.7(a) or consent, and the policy notes / register / informed brief were reworded to say so). The Act has no free-standing minimisation article and the word does not occur in it; the report says DM-01 is derived from s.6(1) + s.4(1).
