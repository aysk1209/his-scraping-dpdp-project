...
*Next, in order (review of 2026-09-17): subject-scoped extraction so minimisation is enforced on the record axis (the fixture's search box is unused; `ExtractedRecord.record_id` is never set); a third `policy` briefing that hands the agent the purpose envelope verbatim, and the flagship model recorded at three samples (the calls are the team's — ~3 days at the free tier's 20/day); a rule-weight sensitivity table in ch7; the Gazette check of the `[s.N — unverified]` tags; a benchmark-protocol document. The hospital dataset is expected before Review-II: run `scripts/check_source.py` first, then `run_pipeline.py --dataset …`.*
...
for the phone), holds none of the four trap tasks in any of twenty runs, and
reproduces its first decision in 15–19 of 32 repeats; ours 20 of 20 and 32 of
32. (2026-09-17: every repeat is now scored — a score is a mean over forty
runs, with the range shown per task; trap and stability counts are over runs.) **OpenAI left out by decision (2026-09-16):** the account's
...
8. ~~Three more literature references~~ — **done 2026-09-15.** [14] Cook 2025
