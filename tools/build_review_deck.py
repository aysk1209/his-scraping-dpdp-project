...
def page_ratio() -> str:
    """The baseline's page loads over ours on the portal run, e.g. 'fourteen' -- from the artefact."""

    import json
    d = json.loads((ROOT / "docs" / "benchmark_results" / "benchmark-portal.json").read_text(encoding="utf-8"))
    pages = {s["short"]: s["cost"]["page_loads"] for s in d["scores"]}
    ratio = round(pages["unconstrained"] / pages["compliance-aware"])
    words = {2: "two", 3: "three", 4: "four", 5: "five", 6: "six", 7: "seven", 8: "eight", 9: "nine", 10: "ten",
             11: "eleven", 12: "twelve", 13: "thirteen", 14: "fourteen", 15: "fifteen", 16: "sixteen"}
    return words.get(ratio, str(ratio))


def ledger_total() -> int:
    """The completion ledger's total, read from PLAN.md §5 -- not typed into the deck."""

    import re
    text = (ROOT / "PLAN.md").read_text(encoding="utf-8")
    m = re.search(r"\| \*\*Total\*\* \| \*\*100\*\* \| \*\*(\d+)\*\*", text)
...
def dataset_rows(name: str = "benchmark-dataset") -> tuple[list[list[str]], str, str] | None:
    """Rows for the dataset slide from ``<name>.json``, or None if that run does not exist.

    Returns (rows, source note, generated date). ``benchmark-dataset`` is the
    rehearsal's run (note 'rehearsal') until a hospital export is run through
    the pipeline; ``benchmark-public`` is the public export's run.
    """

    import json
    path = ROOT / "docs" / "benchmark_results" / f"{name}.json"
    if not path.exists():
        return None
    data = json.loads(path.read_text(encoding="utf-8"))
    sys.path.insert(0, str(ROOT / "src"))
...
def public_ceiling() -> tuple[int, int, int] | None:
    ...
    import json
    path = ROOT / "docs" / "benchmark_results" / "benchmark-public.json"
    if not path.exists():
        return None
    d = json.loads(path.read_text(encoding="utf-8"))
    if not d.get("coverage_needed"):
        return None
    return (d["coverage_needed"], d["coverage_needed"] - d["coverage_in_source"],
...
def build() -> Path:
    ...
    rewrite_body(shape_named(s[8], "Text 2"), [
        ...
        ("Algorithms implemented:", [
            ...
            "another technique above ours). DM-01 on two axes: categories beyond the purpose, and records read beyond the patient's own. "
    ...
    add_text(res, 0.7, 5.15, 11.9, 1.7, [
        f"Reading the table. The baseline loads {page_ratio()} times the pages for the same coverage; its surplus is "
    ...
    # Which real-world run the slide shows: the hospital's, when one exists; otherwise the
    # public export we never saw (the worst case, a hospital export never released, is
    # fully covered by it); otherwise the rehearsal on a hospital-shaped synthetic export.
    ds = dataset_rows()
    source_ds = "hospital" if ds is not None and ds[1] != "rehearsal" else None
    if source_ds is None and (pub := dataset_rows("benchmark-public")) is not None:
        ds, source_ds = pub, "public"
    elif source_ds is None and ds is not None:
        source_ds = "rehearsal"
    n_inserted = 3
    if ds is not None:
        ...
        for sh in list(res4.shapes):
            if sh.name not in ("Text 0", "Text 1", "Image 0"):
                sh._element.getparent().remove(sh._element)
        rehearsal = source_ds == "rehearsal"
        rewrite_plain(shape_named(res4, "Text 1"), [
            "Results & Analysis (contd.) — " + {"rehearsal": "Real Data, Rehearsed", "hospital": "The Hospital Dataset",
                                                "public": "A Public Export We Never Saw"}[source_ds]])
        artefact_ds = "benchmark-public.json" if source_ds == "public" else "benchmark-dataset.json"
        if source_ds == "public":
            ceil = public_ceiling()
            intro = [
                "A hospital's export is personal data, and may never be released to a student project — which is exactly "
                "the risk this work is about. So the dataset path was proven on data we did not generate: the public "
                "Synthea sample (108 synthetic patients, 18 CSV files, one per clinical concept, a 28-column registration "
                "file with SSN, passport and licence columns). It went through the real handling gate (no synthetic "
                "manifest; a provenance note required), a column map written once, and run_pipeline.py --dataset: the same "
                "techniques and recordings, unchanged.",
            ]
            ceiling_txt = (f" The coverage ceiling is measured from the source — {ceil[1]} of the {ceil[0]} fields the "
                           f"tasks need are not in the export, {ceil[2]} are not in the chosen patient's records — so no "
                           f"technique is blamed for what the data lacks." if ceil else "")
            after = [
                "SSN, passport and licence numbers stopped at the adapter: no technique saw them and nothing exported them. "
        ...
        add_text(res4, 0.7, 6.75, 11.9, 0.4,
                 [f"{artefact_ds}, generated {date_ds}; four tasks; the baseline's records-read column is every "
    ...
    rewrite_body(shape_named(chal, "Text 2"), [
        ...
        ("Timeline for completion by Review-III (28.10.2026):", [
            "Week 1: venue chosen with the guide; the manuscript to its template and page budget. Weeks 2–3: the report "
            "assembled — front matter, cross-references, a final read for length. Week 4: every result regenerated from a "
